...
public:
	...
	void setInput(Namespace nsIn, bool dynamicIn, Type left, Type right)
	{
		if (inputHasBeenSet)
		{
			throw A1Error("Same Inputs being parsed '"+getString()+"' more then once", INTERNAL_ERROR, getToken());
		}
	...
	{
		if (!inputHasBeenSet)
			throw A1Error("tried to get inLeftType before input was set", INTERNAL_ERROR, getToken());
		return inLeftType;
	}
	...
	{
		if (!inputHasBeenSet)
			throw A1Error("tried to get inRightType before input was set", INTERNAL_ERROR, getToken());
		return inRightType;
	}
	*/
	...
	Type getReturnType()
	{
		if (!returnType)
		{
			if (!inputHasBeenSet)
			{
				throw A1Error("No Input Set", INTERNAL_ERROR, getToken());
			}
		...
		if (!returnType)
		{
			throw A1Error("Return Type "+getString()+"not defined", INTERNAL_ERROR);
		}
	...
	Action getAction()
	{
		if (!action)
		{
			if (!ns)
			{
				throw A1Error("tried to get action from AST node when input had not been set", INTERNAL_ERROR, getToken());
			}
	...
	void dealWithConstants()
	{
		if (!ns)
		{
			throw A1Error("tried to deal with constants before input was set", INTERNAL_ERROR, getToken());
		}
...
public:
	...
	void resolveAction()
	{
		...
		{
			throw A1Error("AstVoid given non void input", INTERNAL_ERROR, getToken());
		}
...
public:
	...
	static AstNode make(AstNode leftTypeIn, AstNode rightTypeIn, AstNode returnTypeIn, AstNode bodyIn)
	{
		...
		{
			throw A1Error("AstFuncBody made with function input nodes that are not types", INTERNAL_ERROR);
		}
...
public:
	...
	void resolveAction()
	{
		throw A1Error("AstType::resolveAction called, which it shouldn't have been", INTERNAL_ERROR, getToken());
	}
};
...
public:
	...
	void resolveAction()
	{
		throw A1Error("AstActionWrapper::resolveAction called, which it shouldn't have been", INTERNAL_ERROR);
	}
